  ...
  return {
    ...
    transform(code, id) {
      ...
      try {
        ...
        const env = process.env.NODE_ENV ? (options.env ? options.env : null) : null;
        ...
        if (!options.plugins) {
          configPath = options.configPath ? options.configPath : findup.sync(process.cwd(), 'postcss.config.js');
        } else {
          configPath = '';
        }

        let config = options.plugins ? options.plugins : require(path.join(configPath, 'postcss.config.js'));
        ...
        return Promise.all(
          matches.map((css: string) =>
            postcss(outputConfig).process(styleDelimiter ? css.split(styleDelimiter)[1] : css, postcssOptions)
          )
        ).then((transforms: any[]) => {
          transforms.forEach((transform, index) => {
            code = code.replace(
              styleDelimiter ? matches[index].split(styleDelimiter)[1] : matches[index],
              transform.css
            );
          });
          return {
